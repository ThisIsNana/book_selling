...
//				author);
		List<BookSelling> resultList = bookDAO.findAllByContainingRegexp(str);
		List<BookSelling> searchConvert = new ArrayList<>();
		for (BookSelling result : resultList) {
...
//	public BookSellingResponse UpdateBook(BookSelling book) {
		...
		if (count == opCateList.size() && op.get().getInStock() == updateBook.getInStock() && op.get().getPrice() == updateBook.getPrice()) {
